...
def get_action(model, x):
    ...
    with torch.no_grad():
        ...
        action = model.act(x, False)                # 这里是 core 里面的actorCritic的函数 act，是给的确定性动作,没有方差
    return action

def test(path, env_name, task_name, render=True, num_episodes=2000, max_ep_len=1000):
    '''载入模型并且测试'''
    policy = load_model(path)  # 这个载入的policy和logger的save的东西有关
                               # 我save的是ActorCritic这个类，包括类的方法也保留
    env = gym.make(env_name,render=1,task=task_name)

    o, r, d, ep_ret, ep_len, n = env.reset(), 0, False, 0, 0, 0
    # 这个环境reset的时候返回的o不是一个array
    o=o[0]
...
if __name__ == '__main__':
    test('data/sac_unitree_a1/sac_12_ac.pt','quadrupedal-v0', "stairstair")